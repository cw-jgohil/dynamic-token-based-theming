  ...
  for (const [componentName, component] of Object.entries(theme)) {
    ...
    if ("versions" in component) {
      ...
      if (!comp.versions || typeof comp.versions !== "object") {
        issues.push(
          `Component "${componentName}" has invalid versions structure`,
        );
        continue;
      }
      ...
      for (const [versionName, version] of Object.entries(comp.versions)) {
        if (!version || typeof version !== "object") {
          issues.push(
            `Version "${versionName}" in "${componentName}" is invalid`,
          );
          continue;
        }
        ...
        if (ver.tokens && typeof ver.tokens !== "object") {
          issues.push(
            `Tokens in "${componentName}.${versionName}" is not an object`,
          );
        }
        ...
        if (ver.variants) {
          if (typeof ver.variants !== "object") {
            issues.push(
              `Variants in "${componentName}.${versionName}" is not an object`,
            );
          } else {
            for (const [variantName, variant] of Object.entries(ver.variants)) {
              if (!variant || typeof variant !== "object") {
                issues.push(
                  `Variant "${variantName}" in "${componentName}.${versionName}" is invalid`,
                );
                continue;
              }
              ...
              if (!vari.tokens || typeof vari.tokens !== "object") {
                issues.push(
                  `Tokens in variant "${variantName}" of "${componentName}.${versionName}" is invalid`,
                );
              }
            }
          }
      ...
      if (variants) {
        if (typeof variants !== "object") {
          ...
          for (const [variantName, variant] of Object.entries(variants)) {
            if (!variant || typeof variant !== "object") {
              issues.push(
                `Variant "${variantName}" in "${componentName}" is invalid`,
              );
              continue;
            }
            ...
            if (!vari.tokens || typeof vari.tokens !== "object") {
              issues.push(
                `Tokens in variant "${variantName}" of "${componentName}" is invalid`,
              );
            }
          }
        }
